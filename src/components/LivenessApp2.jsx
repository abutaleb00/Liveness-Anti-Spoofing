...
export default function LivenessApp() {
  ...
  const cfg = useMemo(() => ({
    ...
    YAW_FLIP: true, YAW_RAD: 0.10, YAW_MARGIN: 0.04,
    HOLD_FRAMES: 60, HOLD_FRAMES_TURN: 120, MIN_STEP_TIME_S: 0.35,
  ...
  /* ---------- Guidance builder (bilingual + severity) ---------- */
  function buildGuidance(step, st){
    ...
    if(step === "smile"){
      ...
      const wide = wDelta >= cfg.SMILE_W_DELTA
      const lifted = curveDelta >= cfg.SMILE_CURVE_DELTA
      const open = marDelta >= cfg.SMILE_MAR_DELTA2 || (smooth.current.MAR || 0) > Math.max(cfg.MAR_SMILE, st.calib.mar + cfg.SMILE_MAR_DELTA)
      if(!(wide || lifted || open)){
...
/* -------- RAF loop -------- */
  useEffect(() => {
    ...
    const loop = async () => {
      ...
      last = now
      await tick() // internal logic doesn't need dt for now
      if(!freezeLiveRef.current){
...
function GuidanceStrip({ guide }){
  ...
  const toneCls = tone==="ok" ? "border-emerald-500/40 bg-emerald-900/30" : tone==="warn" ? "border-amber-500/40 bg-amber-900/30" : "border-white/10 bg-slate-900/50"
  return (
