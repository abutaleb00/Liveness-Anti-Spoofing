...
export default function LivenessApp() {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        if (!cancelled) { modelRef.current = detector; setModelReady(true); }
      } catch (e) { console.error(e); setError(String(e)); }
    })();
    return () => { cancelled = true; cancelAnimationFrame(rafRef.current); };
  }, []);

  /* ===== Camera control (robust) ===== */
  async function startCamera() {
    try {
      if (!modelReady) return;
      const v = videoRef.current;

      // iOS/Safari inline playback flags
      ...
      await new Promise((resolve) => {
        if (v.readyState >= 1 && v.videoWidth && v.videoHeight) resolve();
      ...
      // Ensure first decoded frame is available
      await new Promise((resolve) => {
  ...
  function buildGuidance(step, st) {
    ...
    if (step === "left") {
      if (yaw < -cfg.YAW_MARGIN) return { titleEn: "Wrong way", titleBn: "ভুল দিক", en: "Rotate LEFT", bn: "বামে ঘোরান", severity: "warn", icon: "↩️" };
      if (yaw < cfg.YAW_RAD) return { titleEn: "Turn Left", titleBn: "বামে তাকান", en: "Rotate a bit more LEFT", bn: "আরও একটু বামে ঘোরান", severity: "progress", icon: "↩️" };
      return { titleEn: "Hold", titleBn: "ধরে রাখুন", en: "Good — hold still", bn: "ভালো — স্থির থাকুন", severity: "ok", icon: "✅" };
    }

    if (step === "right") {
      if (yaw > cfg.YAW_MARGIN) return { titleEn: "Wrong way", titleBn: "ভুল দিক", en: "Rotate RIGHT", bn: "ডানে ঘোরান", severity: "warn", icon: "↪️" };
      if (yaw > -cfg.YAW_RAD) return { titleEn: "Turn Right", titleBn: "ডানে তাকান", en: "Rotate a bit more RIGHT", bn: "আরও একটু ডানে ঘোরান", severity: "progress", icon: "↪️" };
      return { titleEn: "Hold", titleBn: "ধরে রাখুন", en: "Good — hold still", bn: "ভালো — স্থির থাকুন", severity: "ok", icon: "✅" };
    }
    ...
    if (step === "smile") {
      ...
      if (!(wide || lifted || open)) return { titleEn: "Smile", titleBn: "হাসি", en: "Smile wider / lift corners", bn: "আরও হাসুন / মুখের কোণা তুলুন", severity: "progress", icon: "😊" };
      return { titleEn: "Hold", titleBn: "ধরে রাখুন", en: "Looks good — hold", bn: "ভালো — ধরে রাখুন", severity: "ok", icon: "✅" };
    }

    if (step === "blink") {
      if (remainBlink > 0) return { titleEn: "Blink", titleBn: "চোখের পলক", en: `Blink ${remainBlink} more time${remainBlink > 1 ? "s" : ""}`, bn: `আর ${remainBlink} বার পলক দিন`, severity: "progress", icon: "👀" };
  ...
  async function tick() {
    ...
    // NEW: ensure the video actually has dimensions & a decoded frame path
    ...
    if (!faces || faces.length === 0 || !faces[0]?.keypoints?.length) return;
    ...
    <div className="grid lg:grid-cols-5 gap-6">
      <div className="lg:col-span-3 space-y-3">
        <div className="relative rounded-2xl overflow-hidden card min-h-[360px]">
          {/* keep hidden in prod; show during debugging if needed */}
