  ...
  async function startCamera() {
    try {
      ...
      await new Promise((res) => {
        ...
        if (v.readyState >= 1) res(); else v.addEventListener("loadedmetadata", onMeta);
      });
      await v.play();
      ...
      for (let xx = 1; xx < w - 1; xx++) {
        ...
        for (let j = -1; j <= 1; j++) {
          for (let i = -1; i <= 1; i++) {
            ...
            const kv = k[ki++];
            r += sb[idx] * kv; g += sb[idx + 1] * kv; b += sb[idx + 2] * kv;
          }
        }
        const o = (yy * w + xx) << 2;
    ...
    <div className="grid lg:grid-cols-5 gap-6">
      <div className="lg:col-span-3 space-y-3">
        <div className="relative rounded-2xl overflow-hidden card min-h-[360px]">
          ...
          {/* Keep the video in layout so it decodes, but invisible */}
          <video
            ref={videoRef}
