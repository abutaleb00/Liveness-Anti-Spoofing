// LivenessApp.jsx — robust backends + black-canvas fixes
...
const percentile=(arr,p)=>{ if(!arr.length) return 0; const a=[...arr].sort((x,y)=>x-y); const idx=clamp((a.length-1)*p,0,a.length-1); const lo=Math.floor(idx),hi=Math.ceil(idx),t=idx-lo; return a[lo]*(1-t)+a[hi]*t; };
const shuffle=(arr)=>{ const a=[...arr]; for(let i=a.length-1;i>0;i--){const j=Math.floor(Math.random()*(i+1));[a[i],a[j]]=[a[j],a[i]];} return a; };

/* FaceMesh indices */
const IDX={ leftEyeOuter:33,rightEyeOuter:263, L:{left:33,right:133,top1:159,top2:160,bot1:145,bot2:144}, R:{left:362,right:263,top1:386,top2:385,bot1:374,bot2:380}, mouthLeft:61,mouthRight:291,mouthTop:13,mouthBot:14, cheekL:234,cheekR:454,noseTip:4 };
function eyeAspectRatio(pts,kp){const p1=pts[kp.left],p4=pts[kp.right],p2=pts[kp.top1],p6=pts[kp.bot1],p3=pts[kp.top2],p5=pts[kp.bot2]; const A=dist(p2,p6),B=dist(p3,p5),C=dist(p1,p4)+1e-6; return (A+B)/(2*C);}
function mouthAspectRatio(pts){const v=dist(pts[IDX.mouthTop],pts[IDX.mouthBot]); const h=dist(pts[IDX.mouthLeft],pts[IDX.mouthRight])+1e-6; return v/h;}
function rawYaw(pts){const L=pts[IDX.cheekL],R=pts[IDX.cheekR]; if(!L||!R||typeof L.z!=="number"||typeof R.z!=="number") return 0; return Math.atan2((L.z-R.z),Math.abs(R.x-L.x)+1e-6);}

/* PAD helpers */
function planeResidualNorm(pts,keys,faceW){
  const X=[],Z=[]; for(const k of keys){const p=pts[k]; if(!p||typeof p.z!=="number") continue; X.push([p.x/faceW,p.y/faceW,1]); Z.push(p.z/faceW);}
  const n=X.length; if(n<4) return 0; const xtx=[[0,0,0],[0,0,0],[0,0,0]],xtz=[0,0,0];
  for(let i=0;i<n;i++){const [x,y,o]=X[i],z=Z[i];
  ...
  const m=xtx; const det=m[0][0]*(m[1][1]*m[2][2]-m[1][2]*m[2][1])-m[0][1]*(m[1][0]*m[2][2]-m[1][2]*m[2][0])+m[0][2]*(m[1][0]*m[2][1]-m[1][1]*m[2][0]); if(Math.abs(det)<1e-9) return 0;
  ...
  const beta=[inv[0][0]*xtz[0]+inv[0][1]*xtz[1]+inv[0][2]*xtz[2],inv[1][0]*xtz[0]+inv[1][1]*xtz[1]+inv[1][2]*xtz[2],inv[2][0]*xtz[0]+inv[2][1]*xtz[1]+inv[2][2]*xtz[2]];
...
export default function LivenessApp(){
  ...
  const phaseRef=useRef("calibrate"); useEffect(()=>{phaseRef.current=phase;},[phase]);
  ...
  async function switchBackend(to){
    if(tf.getBackend()===to) return true;
    try{
      await tf.setBackend(to); await tf.ready();
      if(modelRef.current?.dispose) { try{ await modelRef.current.dispose(); }catch{} }
      modelRef.current = await createDetector();
      console.info("[TFJS] switched backend →", to);
      return true;
    }catch(e){ console.warn("Failed to switch backend:", to, e); return false; }
  }

  /* -------- model load: prefer WebGL, then WASM -------- */
  useEffect(()=>{
    let cancelled=false;
    (async ()=>{
      try{
        ...
        const order = urlBackend ? [urlBackend,"webgl","wasm","cpu"] : ["webgl","wasm","cpu"];
        for(const b of order){
          try{
            ...
            break;
          }catch(e){ console.warn("Backend init failed:", b, e); }
        }
        if(!modelRef.current) throw new Error("No TFJS backend available");
        if(!cancelled) setModelReady(true);
      }catch(e){ console.error("Detector init failed:", e); setError(String(e)); }
    })();
  ...
  async function startCamera(){
    try{
      ...
      // Ensure first decoded frame before we draw
      await new Promise(res=>{
        let done=false; const ok=()=>{ if(done) return; done=true; res(); };
        const to=setTimeout(()=>res(),900);
        v.addEventListener("playing",()=>{ clearTimeout(to); ok(); },{once:true});
        if("requestVideoFrameCallback" in v){ /* @ts-ignore */ v.requestVideoFrameCallback(()=>{ clearTimeout(to); ok(); }); }
      });
  ...
  async function stopCamera(){
    try{
      const v=videoRef.current; const stream=v?.srcObject;
  ...
  /* ----- PAD + reasons ----- */
  function computePadFromSamples(st){ const arr=st.depthSamples; let depthScore=0;
    if(arr.length>=12){ const z75=percentile(arr.map(o=>o.zRangeN),0.75), p25=percentile(arr.map(o=>o.planeResN),0.25), n50=percentile(arr.map(o=>o.noseProtrusionN),0.50);
      const sZ=scale01(z75,cfg.ZRANGE_GOOD[0],cfg.ZRANGE_GOOD[1]), sP=scale01(p25,cfg.PLANE_GOOD[0],cfg.PLANE_GOOD[1]), sN=scale01(n50,cfg.NOSE_GOOD[0],cfg.NOSE_GOOD[1]);
      depthScore=(0.5*sZ+0.3*sP+0.2*sN)*100;
    }
    const totalSteps=4, passed=["left","right","smile","blink"].reduce((a,k)=>a+(st.passed[k]?1:0),0);
    const action=(passed/totalSteps)*100;
    const timely=st.latencies.filter(t=>t>=cfg.RESPONSE_MIN_S&&t<=cfg.RESPONSE_MAX_S).length;
    const resp=(st.latencies.length?timely/st.latencies.length:0)*100;
    return Math.round(0.60*depthScore+0.25*resp+0.15*action);
  }
  function padReasons(st){
    const arr=st.depthSamples, reasons=[];
    if(arr.length>=12){
      const z75=percentile(arr.map(o=>o.zRangeN),0.75),
            p25=percentile(arr.map(o=>o.planeResN),0.25),
            n50=percentile(arr.map(o=>o.noseProtrusionN),0.50);
      if(z75<cfg.ZRANGE_GOOD[0]) reasons.push("Flat depth (possible screen)");
      if(p25<cfg.PLANE_GOOD[0]) reasons.push("Near-planar surface");
      if(n50<cfg.NOSE_GOOD[0]) reasons.push("Weak nose protrusion");
    }
  ...
  function computeLivenessComposite(st){
    const total=4, passed=["left","right","smile","blink"].reduce((a,k)=>a+(st.passed[k]?1:0),0);
    const step=stepsRef.current[currentStepRef.current];
    const progress = step ? (step==="blink"
      ? clamp((blinkRef.current-st.stepStartBlink)/(blinkTargetRef.current||cfg.BLINK_TARGET),0,1)
      : clamp(holdPct/100,0,1)) : 0;
    const stepScore=((passed+progress)/total)*100;
    const timely=st.latencies.filter(t=>t>=cfg.RESPONSE_MIN_S&&t<=cfg.RESPONSE_MAX_S).length;
    const timing=(st.latencies.length?timely/st.latencies.length:0.65)*100;
    ...
    const live=0.50*stepScore+0.22*timing+0.14*blinkScore+0.14*(smoothness*100);
  ...
  async function tick(){
    ...
    try{
      ...
      // If WASM crashed, fall back to WebGL automatically
      if(tf.getBackend()==="wasm"){
        const ok=await switchBackend("webgl");
        if(ok) return; // next frame will retry on webgl
      }
      return;
    }
    ...
    const n=faces?.length||0; if(n!==faceCount) setFaceCount(n);
    ...
    if(phaseRef.current==="calibrate"){
      ...
      st.calib.earSum+=smooth.current.EAR; st.calib.marSum+=smooth.current.MAR;
      st.calib.mwSum+=smooth.current.WNORM; st.calib.curveSum+=smooth.current.CURVE;
      ...
      if(st.calib.secs>=cfg.CALIB_SECONDS){
        ...
        setPhase("run"); st.stepStartTime=performance.now(); st.stepStartBlink=blinkRef.current;
    ...
    if(done && st.postCapture.pending && !st.postCapture.taken){
      const now=performance.now(), due=st.postCapture.due;
      if(now>=due){
  ...
  function updateStepProgress(){
    ...
    if(step==="left"){ ok=smooth.current.YAW>(cfg.YAW_RAD+cfg.YAW_MARGIN); }
      ...
      if(blinksInStep>=target && timeSinceStart>=minGate){
        st.passed.blink=true; const latency=(performance.now()-st.stepStartTime)/1000; st.latencies.push(latency); nextStep();
      }
      return;
    }
    ...
    const needed=getHoldFramesForStep(step);
    if(timeSinceStart<minGate){ setHoldPct(clamp((st.holdFrames/needed)*100,0,100)); return; }

    if(ok) st.holdFrames++; else st.holdFrames=Math.max(0,st.holdFrames-1);
    setHoldPct(clamp((st.holdFrames/needed)*100,0,100));

    if(st.holdFrames>=needed){
      st.holdFrames=0; st.passed[step]=true;
      const latency=(performance.now()-st.stepStartTime)/1000; st.latencies.push(latency);
      nextStep();
    }
  }
  ...
  function nextStep(){
    ...
    setCurrentStep(i=>{
      ...
      if(n>=stepsRef.current.length){
        ...
        st.stepStartTime=performance.now(); st.stepStartBlink=blinkRef.current; st.holdFrames=0;
      }
      return n;
    });
  }
  ...
  function scheduleFinalizeResults(){
    ...
    setTimeout(()=>{
      const st=stateRef.current; const padNow=computePadFromSamples(st);
  ...
  function updateDepthSamples(pts){ const f=extractDepthFeatures(pts); stateRef.current.depthSamples.push(f); if(stateRef.current.depthSamples.length>200) stateRef.current.depthSamples.shift(); smooth.current.depthVar=f.zRangeN||0; }

  function drawMesh(ctx,face){ const kp=face.keypoints; ctx.save(); ctx.fillStyle="rgba(45,212,191,0.5)"; for(let i=0;i<kp.length;i+=8){ const p=kp[i]; ctx.beginPath(); ctx.arc(p.x,p.y,1.6,0,Math.PI*2); ctx.fill(); } ctx.restore(); }
  ...
  async function grabBestFrameCanvas(video){
    ...
    try{
      ...
      if(track && "ImageCapture" in window){
        ...
        const c=make(bmp.width,bmp.height), cx=c.getContext("2d");
        cx.save(); cx.translate(c.width,0); cx.scale(-1,1);
        cx.drawImage(bmp,0,0); cx.restore();
        return c;
      }
    }catch{}
    const c=make(video.videoWidth,video.videoHeight), cx=c.getContext("2d");
    cx.save(); cx.translate(c.width,0); cx.scale(-1,1);
    cx.drawImage(video,0,0); cx.restore();
    return c;
  }

  function enhanceCanvasInPlace(ctx,w,h){
    const img=ctx.getImageData(0,0,w,h), d=img.data, n=d.length; let rSum=0,gSum=0,bSum=0,count=n/4;
    for(let i=0;i<n;i+=4){ rSum+=d[i]; gSum+=d[i+1]; bSum+=d[i+2]; }
    const rMean=rSum/count,gMean=gSum/count,bMean=bSum/count, avg=(rMean+gMean+gMean/0+ bMean)/3 || 1; // keep avg numeric
    ...
    const total=count, lowCut=Math.round(total*0.02), highCut=Math.round(total*0.98);
    let acc=0, low=0, high=255;
    for(let v=0;v<256;v++){ acc+=hist[v]; if(acc>=lowCut){ low=v; break; } }
    acc=0; for(let v=255;v>=0;v--){ acc+=hist[v]; if(acc>=total-highCut){ high=v; break; } }

    const eps=Math.max(1,high-low), gain=255/eps, bias=-low*gain, gamma=0.95;
    for(let i=0;i<n;i+=4){
      let r=clamp((d[i]*gain+bias),0,255), g=clamp((d[i+1]*gain+bias),0,255), b=clamp((d[i+2]*gain+bias),0,255);
    ...
    const src=ctx.getImageData(0,0,w,h), sb=src.data, blur=new Uint8ClampedArray(sb.length), k=[1,2,1,2,4,2,1,2,1];
    for(let yy=1;yy<h-1;yy++){
      for(let xx=1;xx<w-1;xx++){
        let r=0,g=0,b=0,ki=0;
        for(let j=-1;j<=1;j++){ for(let i=-1;i<=1;i++){ const idx=((yy+j)*w+(xx+i))<<2, kv=k[ki++]; r+=sb[idx]*kv; g+=sb[idx+1]*kv; b+=sb[idx+2]*kv; } }
        const o=(yy*w+xx)<<2; blur[o]=r>>4; blur[o+1]=g>>4; blur[o+2]=b>>4; blur[o+3]=255;
      }
    }
    const amount=0.55;
    for(let y=1;y<h-1;y++){
      for(let x=1;x<w-1;x++){
        const i4=(y*w+x)<<2, r=sb[i4], g=sb[i4+1], b=sb[i4+2], br=blur[i4], bg=blur[i4+1], bb=blur[i4+2];
  ...
  async function copySnapshotBase64(){
    const shot=await capturePassport(); if(!shot?.base64) return;
    ...
    <div className="grid lg:grid-cols-5 gap-6">
      <div className="lg:col-span-3 space-y-3">
        <div className="relative rounded-2xl overflow-hidden card min-h-[360px]">
          ...
          {/* keep a real size so video decodes even when hidden */}
          <video
            ref={videoRef}
            className="w-[320px] h-[180px] absolute opacity-0 pointer-events-none"
            muted
            playsInline
            autoPlay
          />
      ...
      <div className="lg:col-span-2 space-y-4">
        {cameraOn && (
          <>
            <LiveCard getIndicator={()=>{
              const st=stateRef.current; const score=computeLivenessComposite(st);
              const label=score>=75?"Strong":score>=50?"In Progress":"Low";
              const reasons=liveReasons(st); return {score,label,reasons};
            }} />
            <PadCard getIndicator={()=>{
              const st=stateRef.current; const score=computePadFromSamples(st);
              const label=score>=75?"Live":(score<50?"Block":"Suspicious");
              const reasons=padReasons(st); return {score,label,reasons};
            }} />
          </>
        )}
        ...
        <div className="card p-4">
          ...
          <ul className="mt-3 space-y-2">
            {steps.map((s)=>{
              const isCurrent=steps[currentStep]===s, passed=stateRef.current.passed[s], data=INSTRUCTIONS[s];
              return (
...
/* -------- small components -------- */
function Metric({label,value,ok}){ return (
...
function PadCard({getIndicator}){ const ind=getIndicator();
  ...
  return (
    <div className="card p-4">
      ...
      <div className="mt-2 text-sm">
        ...
        {ind.reasons?.length>0 ? <ul className="list-disc pl-5 text-slate-300 mt-1">{ind.reasons.map((r,i)=><li key={i}>{r}</li>)}</ul>
          : <div className="text-slate-400">No issues detected so far.</div>}
      </div>
    </div>
  );
}
function LiveCard({getIndicator}){ const ind=getIndicator();
...
function PromptHUD({phase,calibPct,cameraOn,cameraReady,faces,done,finalPass,stepKey}){
  const commonPanel={ initial:{opacity:0,y:-12,scale:0.98}, animate:{opacity:1,y:0,scale:1,transition:{type:"spring",stiffness:220,damping:20}}, exit:{opacity:0,y:-8,scale:0.98,transition:{duration:0.18}} };
  const chipFx={ initial:{opacity:0,y:12}, animate:{opacity:1,y:0,transition:{type:"spring",stiffness:200,damping:20}}, exit:{opacity:0,y:12,transition:{duration:0.18}} };
          ...
          {phase==="run" && !done && step && (
            <motion.div key={stepKey} {...commonPanel} className="px-5 py-4 rounded-2xl bg-slate-900/80 backdrop-blur border border-white/10 shadow-xl w-[min(92vw,360px)]">
              <div className="text-xl font-semibold text-slate-50 flex items-center gap-2">
                <span className="text-2xl" aria-hidden>{step.icon}</span><span>{step.titleEn}</span>
              </div>
...
function liveReasons(st){
  const reasons=[];
  if(st.latencies.length){ const timely=st.latencies.filter(t=>t>=0.18&&t<=3.0).length; if(timely/st.latencies.length<0.7) reasons.push("Responses too fast/slow"); }
  if(st.blinkAmps.length){ const med=percentile(st.blinkAmps,0.5); if(med<0.05) reasons.push("Blinks too shallow"); }
